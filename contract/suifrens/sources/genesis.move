...
module suifrens::genesis {
    ...
    public struct Mint has key, store {
        id: UID,
        /// The fee that user has to pay for minting.
        /// Can be changed through method that requires admin capability
        minting_price: u64,
        /// The has stored in the object to provide a seed for minting
    ...
    public fun mint<T>(
        ...
        // create seed based on mutiple osurces: clock, fresh uid and an inner hash.
    ...
    /// Private functions for handling coins received from miximng payments.
    ...
    /// Must-have function available for the Capy Admin call.
    ...
    /// Registers gene difinitions for the Genesis App
    ...
    /// Withdraw profits from the app as a sigle coin
    ...
    public fun take_profits(
        ...
        ctx: &mut TxContext
            ...
            // Take a transferable `Coin` from a Balanace
            coin::take(&mut self.profits, amount, ctx)
        }

    /// Modifies the minting linit, Requires Admin Capabilities
    public fun update_minting_limit(
        _: &AdminCap,
        self: &mut Mint,
        new_mixing_limit: u8
    ) {
