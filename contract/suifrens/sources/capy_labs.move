...
module suifrens::capy_labs {
    ...
    use sui::tx_context::{fresh_object_address};
    ...
    /// Trying to mix, when mix limnit has been reached.
    const EReachedMixingLimit: u64 = 1;
    /// Trying to mix, when cool down period still exists.
    const EStillInCoolDownPeriod: u64 = 2;
    /// The amount paid does not match the expect.
    const EAmountIncorrect: u64 = 3;
    /// For when there's no profits to claim.
    const ENoMixingProfits: u64 = 4;
    /// When ask to generate genes that are not equal to 32, th elength of the sha3_256.
    const EGenesLength: u64 = 5;

    // === Constants ===

    /// Number of meaningful genes. also marks the length
    /// of th ehash used in application: blake2b256.
    /// Currently on the first 5 {EGenesLength} are used for: skin, amin, secondary, expresision, and ears.
    /// But We mutate them until we enable them in the application logic.
    const GENES: u64 = 32;

    /// According to bussiness rules, in the mix process, during gene computation there is a 40% propability
    /// to get a Gene from Parent A, a 40% probabilty to get a Gene from Parent B and 20& prob to get a common trait.
    ...
    /// There is a breeding cool doen, which is the amount of time a SuiFren needs to wait until it
    /// can breed again. To start with, this value will be 1 epoch.
    ...
    /// as well as the storage for the main informnation about the gamestate.
    public struct CapyLabsApp has key {
        id: UID,
        /// Update every time a new SuiFren is minted. Used to provide a source
        /// of entropy for the gene science algoritm.
        inner_hash: vector<u8>,
        /// Mixing limit set per SUiFren on the first mix event.
        mixing_limit: u8,
        /// SuiFrens can be mixed with constrained frequency. If a miux happens,
        /// then a nmumber of epochs = cool_doen_period should pass for th enext
        /// generation to be allowed
        cool_down_period: u64,
        /// The fee that user has to pay for mixing. Can be changed through method
        /// that requires an admin capabilty.
        mixing_price: u64,
        /// The profits collected by the applicaitno. Can only be claimed by the
        /// SF Admin.
        profits: Balance<SUI>
    }

    /// Dynamic field Keys: Stores thew mixing  limit. Can be used and read by
    /// other applicaitons.
    public struct MixLimitKey has copy, store, drop {}
    /// Dynamic field Keys: Strores the last epoch a SuiFren was mixed.
    /// Can be userd and read by other applicaitons.
    public struct LastEpochMixedKey has copy, store, drop {}
    /// Dynamic field Key: Stores vector og parents (usually two :))
    public struct ParentKey has copy, store, drop {}


    // === funcitons ===

    /// Initialize the applicaiton by creating and sharing the CapyLabsApp object.
    /// Authorization of the application needs to be performed dynam,ically by the
    /// SF Admin.
    ...
    public fun mix<T>(
        ...
        // Deal with Cool Down Period Bussiness Rules
        {
            ...
            if (option::is_some(&sf1_last_epoch_mixed)) {
                ...
                let epochs_passes_1 = current_epoch - last_epoch_mixed_1;
                assert!(epochs_passes_1 >= self.cool_down_period, EStillInCoolDownPeriod)
            };
        ...
        // Create seed based on mutiple source, fresh uid and an inner hash.
        ...
        let mut suifren = sf::mint<T>(&mut self.id, new_generation, genes, attributes, clock, ctx);
    ...
    /// Private funciton for handling coins received from mixing payments.
    /// It breaks flow with an exception message if the payment coin does not cover price.
    fun handle_payment(
        self: &mut CapyLabsApp,
        paid: &mut Coin<SUI>,
        ctx: &mut TxContext
    ...
    /// External accessor for property defined this application.
    ...
    /// Private: get or set alst epoch mixed.
    ...
    /// Withdraw profits from the App as a single coin (accumulated as DOF).
    ...
    /// Authorize the app to mint new SuiFrens. Can obly be performed by the SF Admin.
    ...
    /// Deauthorize thr app to mint new SuiFrens. Can only be performed by the SF Admin.
    /// Functin can not be removed by package upgrades, hence SF Admin will always have
    ...
    /// Registers gene definitions for the CaptLabs App (and a specicfic type SF).
    public fun add_genes<T>(
        _: &AdminCap,
        self: &mut CapyLabsApp,
        genes_bcs: vector<u8>
    ) {
    ...
    public fun set_mixing_price(
        _: &AdminCap,
        self: &mut CapyLabsApp,
        new_mixing_price: u64
    ) {
